...
let rec conv_term (t:A.term): T.t =
  ...
  | A.App (f,l) ->
    let l = FList.map conv_term l in
  ...
  | A.App_poly (f,tys,l) ->
    let tys = FList.map conv_ty tys in
    let l = FList.map conv_term l in
    ...
    let l = FList.map
        (function
          ...
          | A.Match_case (s,vars,t) ->
            let vars = FList.map (fun v->T.V v) vars in
            T.Match_case (s,vars,conv_term t))
        l
    in
    T.match_ u l
  | A.Let (l,u) ->
    let l = FList.map (fun (v,t) -> T.V v, conv_term t) l in
    let u = conv_term u in
    T.let_ l u
  | A.Fun (v,t) ->
    let v = conv_var v in
  ...
  | A.And l -> T.and_ (FList.map conv_term l)
  | A.Or l -> T.or_ (FList.map conv_term l)
...
let conv_decl (d:A.ty A.fun_decl): string * T.t =
  let tyvars = FList.map conv_tyvar d.A.fun_ty_vars in
  let ty_args = FList.map conv_ty d.A.fun_args in
...
let conv_def (d:A.typed_var A.fun_decl): string * T.typed_var list * T.t =
  let tyvars = FList.map conv_tyvar d.A.fun_ty_vars in
  let vars = FList.map conv_var d.A.fun_args in
  let ty_args = FList.map (CCFun.compose snd conv_ty) d.A.fun_args in
  let ty_ret = conv_ty d.A.fun_ret in
  let ty = T.forall_ty tyvars (T.fun_ty ty_args ty_ret) in
...
let convert (st:A.statement): UA.statement list =
  ...
  | A.Stmt_prove (tyvars,g) ->
    (* goal *)
    let tyvars = FList.map conv_tyvar tyvars in
    ...
    let cstors =
      FList.map
        (fun c ->
          let args =
            c.A.cstor_args
            |> FList.map (CCPair.map CCOpt.return conv_ty) in
          c.A.cstor_name, args)
        cstors
    in
    let l = [{UA.
  ...
  | A.Stmt_funs_rec {A. fsr_decls; fsr_bodies } ->
    ...
    let l = FList.map2 (conv_def ?loc) fsr_decls fsr_bodies in
    [UA.def ?loc l]
